...
class CommandBusSubscriber extends subscriptionImplementor_1.SubscriberImplementor {
    ...
    onNextAsync(event) {
        return __awaiter(this, void 0, void 0, function* () {
            constants_1.logger.debug("CommandBus.onNextAsync is called");
            return (0, filterDatabaseEvent_1.filterDatabaseEvent)(yield this.linkedPublisher.notifyAsync(event)).unwrapOrElse(() => {
                constants_1.logger.error("CommandBus: DatabaseEvent filtration leads to all error");
                return new databaseErrorEvent_1.default("The action is failed to be executed", 400);
            });
        });
    }
    onNext(event) {
        constants_1.logger.debug("CommandBus.onNext is called");
        super.onNext(event);
...
class CommandBus extends subscriptionImplementor_1.PublisherImplementor {
    constructor(...publishers) {
        ...
        constants_1.logger.debug("Initialized CommandBus");
    }
}
